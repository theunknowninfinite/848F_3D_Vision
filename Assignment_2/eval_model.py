...
def evaluate_model(args):
    ...
    for step in range(start_iter, max_iter):
        ...
        if iter_counter==50:
            iter_counter=0
            if args.type == "vox":
                    print(f"Saving {args.type}  {step} image")
                    # print(feed_dict)
                    image=feed_dict["images"]
                    image=image.squeeze(0).cpu().numpy()
                    image = (image - image.min()) / (image.max() - image.min()) * 255.0
                    image=np.array(Image.fromarray(image.astype('uint8')).resize((image.shape[1]*2,image.shape[0]*2)))
                    plt.imsave(f"output_images/{args.type}/{args.type}_{step}_img.jpg",image)
                    renderer.render_vox(torch.sigmoid(predictions[0]),(str(step)+"pred"+args.type),output_type=args.type,cam_dist=4,image_size=512,cam_ele=0,colors=[1, 1, 1])
                    vox_gt = feed_dict['voxels'].to(args.device)
                    renderer.render_vox(vox_gt[0], (str(step)+"gt"+args.type),output_type=args.type,cam_dist=4,image_size=512,cam_ele=30)
                    # exit()
            if args.type == "point":
                print(f"Saving {args.type}  {step}  image")
                image=feed_dict["images"]
                ...
                image=np.array(Image.fromarray(image.astype('uint8')).resize((image.shape[1]*2,image.shape[0]*2)))
                plt.imsave(f"output_images/{args.type}/{args.type}_{step}_img.jpg",image)
                renderer.render_point_cloud(predictions, (str(step)+"pred"+args.type),output_type=args.type,cam_dist=2,image_size=512)
                gt_points = sample_points_from_meshes(mesh_gt, args.n_points)
                renderer.render_point_cloud(gt_points, (str(step)+"gt"+args.type),output_type=args.type,cam_dist=2,image_size=512)
            if args.type == "mesh":
                print(f"Saving {args.type} {step} image")
                image=feed_dict["images"]
                image=image.squeeze(0).cpu().numpy()
                ...
                image=np.array(Image.fromarray(image.astype('uint8')).resize((image.shape[1]*2,image.shape[0]*2)))
                plt.imsave(f"output_images/{args.type}/{args.type}_{step}_img.jpg",image)
                renderer.render_mesh(predictions, (str(step)+"pred"+args.type),output_type=args.type,cam_dist=2,image_size=512)
                renderer.render_mesh(mesh_gt.to(args.device), (str(step)+"gt"+args.type),output_type=args.type,cam_dist=2,image_size=512)
