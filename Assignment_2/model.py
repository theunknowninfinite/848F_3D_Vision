...
#Credits given in readme and report. (https://arxiv.org/pdf/1901.11153.pdf)
class voxel_decoder(nn.Module):
    def __init__(self):
        super(voxel_decoder, self).__init__()
...
class PointDecoder(nn.Module):
    def __init__(self,  n_points,input_size=512):
    ...
    def forward(self, x):
        ...
        output = output.view(-1, self.n_point, 3)
        return output
...
class MeshDecoder(nn.Module):
    def __init__(self, input_size):
        ...
        self.layers = nn.Sequential(
            ...
            # nn.Linear(2048, 2048),
            # nn.ReLU(),
            nn.Linear(2048, self.input_size * 3)
        )

    def forward(self, x):
        output = self.layers(x)
        output = output.view(-1, self.input_size, 3)
        return output
