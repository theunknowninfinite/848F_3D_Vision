...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        exp_density=torch.exp((-rays_density*deltas))
        exp_density_expanded = torch.cat((torch.ones_like(exp_density[:, :1, :]), exp_density[:, :-1, :]), dim=1)
        transmittance = torch.cumprod(exp_density_expanded, dim=1)
        # TODO (1.5): Compute weight used for rendering from transmittance and alpha
        weights= transmittance*(1-exp_density)
        return weights
