...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    rand_idx = torch.randperm(xy_grid.size(0))
    xy_grid_sub = xy_grid[rand_idx[:n_pixels]]
    # return xy_grid_sub.reshape(-1, 2)[:n_pixels]\
    return xy_grid_sub.to(device)
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    unprojected_pts=camera.unproject_points(ndc_points,from_ndc=True).to(device)
    ...
    rays_d= torch.nn.functional.normalize((unprojected_pts-rays_o)).to(device)
