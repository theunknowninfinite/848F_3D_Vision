...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            render_points("images/ray_sampled.png", ray_bundle.sample_points.reshape(1, -1, 3))
        ...
        if cam_idx == 2 and file_prefix == '':
            depth=out["depth"]
            plt.imsave("images/depth.png",depth.cpu().detach().numpy().reshape(image_size[1],image_size[0]))
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            color=out["feature"]
            color=color.reshape(-1,3).to("cuda:0")
            loss = torch.nn.functional.mse_loss(color,rgb_gt)
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            loss = torch.nn.MSELoss()(color,rgb_gt)
