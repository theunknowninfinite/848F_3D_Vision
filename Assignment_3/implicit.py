...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        num_layers=8
        # inspired by https://github.com/kwea123/nerf_pl/tree/master
        for i in range(num_layers):
            if i == 0:
                self.layers.append(torch.nn.Linear(embedding_dim_xyz,layer_width).to(device))
            elif i==5:
                self.layers.append(torch.nn.Linear(embedding_dim_xyz+layer_width,layer_width).to(device))
            else:
                self.layers.append(torch.nn.Linear(layer_width,layer_width).to(device))
        self.relu=torch.nn.ReLU()
        self.sigma = torch.nn.Linear(layer_width, 1)
        self.rgb_layer = torch.nn.Sequential(
                        torch.nn.Linear(layer_width, 3),
                        torch.nn.Sigmoid())

    def forward(self, ray_bundle):

            xyz=ray_bundle.sample_points
            xyz_embedding = self.harmonic_embedding_xyz(xyz).to(device)


            x = xyz_embedding.to(device)
            for i, layer in enumerate(self.layers):
                x=layer(x)
                x=self.relu(x)
                if i == 4:
                    x = torch.cat([x, xyz_embedding], dim=-1).to(device)


            density= self.sigma(x)
